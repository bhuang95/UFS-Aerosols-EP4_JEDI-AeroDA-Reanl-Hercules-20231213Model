...
DATAHPSSDIR=${ARCHHPSSDIR}/${PSLOT}/dr-data-longfcst/${CDATE}  #${CY}/${CY}${CM}/${CYMD}/
...
if [ -s ${CNTLDIR_ATMOS_RT_CDATE} ]; then
	...
	echo "Set TARALLRST = YES in the XML file"
	exit 100
...
#            tar -P -cvf ${TARFILE} $(cat list.${FHR})
#            ERR=$?
#            if [ ${ERR} -ne 0 ]; then
#                echo "HTAR cntl restart data failed at ${CDATE} and ${FHR}" >> ${HPSSRECORD}
#                exit ${ERR}
...
if [ ${ERR} -eq 0 ]; then
    echo "HTAR is successful at ${CDATE}"
    cd ${TMPDIR}
    echo "Globus failed at ${CDATE}" > ${GLBUSRECORD}
else
    echo "HTAR failed at ${CDATE}" >> ${HPSSRECORD}
    exit ${ERR}
fi
exit ${ERR}
